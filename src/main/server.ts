...
const FUNCTION_PROPERTIES = ['length', 'name', 'arguments', 'caller', 'prototype'];
  ...
  if (ref !== undefined && ref.deref() === undefined) {
    ...
    if (!fi.webContents.isDestroyed()) {
      try {
          ...
          IPC_MESSAGES.RENDERER_RELEASE_CALLBACK,
          fi.id[0],
          fi.id[1],
        );
      } catch (error) {
...
function setCachedRendererFunction(
  id: RendererFunctionId,
  wc: WebContents,
  frameId: number,
  value: CallIntoRenderer,
) {
  // eslint-disable-next-line no-undef
...
const removeRemoteListenersAndLogWarning = (
  sender: any,
  callIntoRenderer: (...args: any[]) => void,
) => {
          ...
          if (!sender.isDestroyed()) {
            try {
              succeed =
                (sender.sendToFrame(
                  frameId,
                  IPC_MESSAGES.RENDERER_CALLBACK,
                  contextId,
                  meta.id,
                  valueToMeta(sender, contextId, args),
...
const isRemoteModuleEnabledImpl = function (contents: WebContents) {
  ...
  return (webPreferences as any).enableRemoteModule != null
    ? !!(webPreferences as any).enableRemoteModule
    : false;
};
...
const handleRemoteCommand = function (
  channel: string,
  handler: (event: IpcMainEvent, contextId: string, ...args: any[]) => void,
) {
...
export function initialize() {
  if (initialized) throw new Error('@electron/remote has already been initialized');
  initialized = true;
  handleRemoteCommand(
    IPC_MESSAGES.BROWSER_WRONG_CONTEXT_ERROR,
  ...
  handleRemoteCommand(
    IPC_MESSAGES.BROWSER_GET_BUILTIN,
  ...
  handleRemoteCommand(
    IPC_MESSAGES.BROWSER_GET_GLOBAL,
  ...
  handleRemoteCommand(
    IPC_MESSAGES.BROWSER_GET_CURRENT_WEB_CONTENTS,
  ...
  handleRemoteCommand(
    IPC_MESSAGES.BROWSER_MEMBER_CONSTRUCTOR,
  ...
  handleRemoteCommand(
    IPC_MESSAGES.BROWSER_MEMBER_CALL,
    function (event, contextId, id, method, args) {
      args = unwrapArgs(event.sender, event.frameId, contextId, args);
      const object = objectsRegistry.get(id);
